  ...
  return (
    <div className="ia-wrap config-wrap">
      ...
      <div className="ia-content">
        ...
        {tab === "operacional" && (
          <SecaoOperacional onRefresh={loadAll} />
        )}
        {tab === "clientes" && (
          <SecaoClientes
            clientes={clientes}
...
function SecaoOperacional({ onRefresh }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [operConfig, jbs, evts] = await Promise.all([
        cfg.getOperacional().catch(() => ({})),
        cfg.getJobs().catch(() => []),
        cfg.getAuditoriaEventos({ limit: 30 }).catch(() => []),
      ]);
      setConfig(operConfig || {});
      setJobs(jbs || []);
      setEventos(evts || []);
    } catch (e) {
      console.error("SecaoOperacional load:", e);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleUpdateConfig = async (patch) => {
    setSaving(true);
    try {
      const updated = await cfg.putOperacional(patch);
  ...
  const handleSyncContatos = async () => {
    setAction("sync-contatos");
    try {
      await cfg.postJobSyncContatos();
      load();
    } catch (e) {
      alert(e?.response?.data?.error || "Erro ao enfileirar");
    } finally {
      setAction(null);
    }
  };
  ...
  const handleSyncFotos = async () => {
    setAction("sync-fotos");
    try {
      await cfg.postJobSyncFotos();
      load();
    } catch (e) {
      alert(e?.response?.data?.error || "Erro ao enfileirar");
    } finally {
  ...
  const handleRetry = async (id) => {
    try {
      await cfg.retryJob(id);
      load();
    } catch (e) {
      alert(e?.response?.data?.error || "Erro ao retentar");
    }
  };

  if (loading) return <p className="ia-muted">Carregando...</p>;

  return (
    <div className="ia-section">
      <h4>Configurações Operacionais</h4>
      <p className="ia-muted" style={{ marginBottom: 16 }}>
        Controles de sincronização, modo seguro e fila de jobs.
      </p>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        <div className="ia-field">
          <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Switch
              checked={!!config?.modo_seguro}
              onChange={(v) => { if (saving) return; handleUpdateConfig({ modo_seguro: v }); }}
            />
            <span>Modo seguro</span>
          </label>
          <p className="ia-muted">Operações conservadoras com limites e intervalos.</p>
        </div>

        <div className="ia-field">
          <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Switch
              checked={!!config?.processamento_pausado}
              onChange={(v) => { if (saving) return; handleUpdateConfig({ processamento_pausado: v }); }}
            />
            <span>Processamento pausado</span>
          </label>
          <p className="ia-muted">Pausa todos os jobs de sync até retomar.</p>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button
            type="button"
            className="ia-btn ia-btn--primary"
            onClick={handleSyncContatos}
            disabled={action === "sync-contatos"}
          >
            {action === "sync-contatos" ? "Enfileirando…" : "Sync contatos"}
          </button>
          <button
            type="button"
            className="ia-btn ia-btn--outline"
            onClick={handleSyncFotos}
            disabled={action === "sync-fotos"}
          >
            {action === "sync-fotos" ? "Enfileirando…" : "Sync fotos"}
          </button>
          <button
            type="button"
            className="ia-btn ia-btn--outline"
          ...
          <button type="button" className="ia-btn ia-btn--outline" onClick={load}>
            Atualizar
          </button>
        </div>

        <div>
          <h5 style={{ margin: "16px 0 8px" }}>Fila de jobs</h5>
          <div style={{ maxHeight: 200, overflowY: "auto" }}>
            {jobs.length === 0 ? (
              <p className="ia-muted">Nenhum job.</p>
            ) : (
              jobs.map((j) => (
                <div key={j.id} style={{ padding: "6px 0", borderBottom: "1px solid #e2e8f0", fontSize: 13 }}>
                  <span>#{j.id}</span> {j.tipo} — {j.status}
                  ...
                  <span className="ia-muted" style={{ marginLeft: 8 }}>
                    {j.criado_em ? new Date(j.criado_em).toLocaleString("pt-BR") : ""}
                  </span>
                </div>
              ))
            )}
        ...
        <div>
          <h5 style={{ margin: "16px 0 8px" }}>Eventos operacionais</h5>
          <div style={{ maxHeight: 200, overflowY: "auto" }}>
            {eventos.length === 0 ? (
              <p className="ia-muted">Nenhum evento.</p>
            ) : (
              eventos.map((e) => (
                <div key={e.id} style={{ padding: "6px 0", borderBottom: "1px solid #e2e8f0", fontSize: 12 }}>
                  <strong>{e.tipo}</strong> — {e.evento} — {e.criado_em ? new Date(e.criado_em).toLocaleString("pt-BR") : ""}
                </div>
              ))
            )}
          </div>
