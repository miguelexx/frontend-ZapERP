...
Lista `/disparo` (`DisparoMensagens.jsx`): cards com rodapé de ações visíveis (Abrir/Config/Acompanhar + **Excluir**). `DELETE /api/disparo/campanhas/:id` remove a campanha e a fila (`company_id` do JWT); bloqueia `em_execucao`. Arquivar continua disponível. Filtros incluem `pronta` e `pausada`. Visível no menu/rotas só com `canAcessarDisparo`: módulo Campanhas ativo na empresa (`modulo_campanhas_ativo`) **e** admin/`disparo.ver`. Em Configurações → Geral o admin informa a senha e clica **Ativar** (`PUT /config/empresa` + `senha_modulo_campanhas`; a senha não vai no bundle). A flag entra no `authStore` na hora: o item **Campanhas** do menu e o chip da lista aparecem sem F5, e a UI abre `/disparo`. Desativar também é imediato (sem senha). Não ligar envio real pelo FE; o worker live é decisão de backend/ops.
