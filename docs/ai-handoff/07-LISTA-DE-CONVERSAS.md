...
- **Setor em tempo real (CONFIRMADO 2026-09-02):** quando o cliente escolhe um setor (URA/chatbot) ou a conversa é transferida de setor, o card some na hora para quem **não** pertence àquele departamento. **Admin** (`role`/`perfil` `admin` ou `administrador`) vê tudo. Supervisor **não** é admin nessa regra (igual ao GET `/chats`). Sem setor → todos veem. Conversa assumida por mim permanece. `viewerCanSeeConversationRow` + `shouldRemoveChatFromViewerList` no socket (`conversa_atualizada` / `atualizar_conversa` / 403 no GET `:id`) e em `computeChatsFiltrados`. Fechar a thread se o setor ficou inacessível **não** encerra o atendimento. Transferir setor no backend também emite `conversa_atualizada` na room `empresa_*` (quem perdeu o setor precisa do payload para dropar; `emitirConversaAtualizada` só entrega a quem ainda pode ver).
