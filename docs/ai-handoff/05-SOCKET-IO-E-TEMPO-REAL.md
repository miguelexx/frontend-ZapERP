...
Filtro: `shouldIgnoreByCompany(payload)` compara `payload.company_id|empresa_id` com o user. Evento de outro tenant **não** deve mutar stores. Isso é defesa de UI, não segurança. Recorte de setor no client: `shouldRemoveChatFromViewerList` (admin vê tudo; atendente só o próprio departamento e não vê individual assumida por outro). Mudança de setor (URA/transferência) chega em `conversa_atualizada` também pela room da empresa — quem perdeu o setor precisa do payload para dropar; o emit só para quem ainda vê deixava o card até F5.
